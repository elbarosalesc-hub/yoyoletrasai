...
export default function OwnerPlatformManager({ accessToken }) {
  ...
  return <section className="owner-platform-panel">
    <div className="owner-platform-hero">
      <div><span className="eyebrow">PERFIL PROPIETARIO · APLICACIÓN MAESTRA</span><h3>Centro de Control YoYoLetrasAI</h3><p>{profile.ownerApplication.purpose}</p></div>
      <div className="owner-protection"><strong>Producción protegida</strong><span>Se acumulan y validan mejoras en GitHub; el despliegue queda reservado para un lote listo.</span></div>
    </div>
    ...
    <div className="owner-control-grid">
      ...
      <article><span>Cache plataforma</span><strong>{profile.cache.defaultStrategy}</strong><State ok={profile.cache.implemented} readyLabel="Implementado" /></article>
      ...
      <article><span>Preparación de release</span><strong>{data.checklist.releaseReadyByPolicy ? 'Política cumplida' : 'En progreso'}</strong><State ok={data.checklist.releaseReadyByPolicy} readyLabel="Base lista" /></article>
    </div>

    <section className="owner-platform-card">
      <div><span className="eyebrow">SUPERIORIDAD EDUCATIVA</span><h4>Programa permanente de excelencia</h4><p>La plataforma se audita y mejora transversalmente, no sólo en IA. Estas dimensiones forman parte del perfil propietario y de la hoja de ruta.</p></div>
      <div className="owner-module-list">{profile.excellence.dimensions.map((dimension) => <article key={dimension.id}><b>{dimension.label}</b><small>{dimension.objective}</small><State ok={dimension.continuousImprovement} readyLabel="Mejora continua" /></article>)}</div>
    </section>

    <div className="owner-platform-columns">
      <section className="owner-platform-card">
        <div><span className="eyebrow">MÓDULOS</span><h4>Administración de los 19 módulos</h4><p>Renombrar, mostrar u ocultar, cambiar íconos y generar recursos por módulo desde una sola aplicación, habilitando escrituras por etapas.</p></div>
      ...
      <section className="owner-platform-card">
        <div><span className="eyebrow">DISEÑO Y MARCA</span><h4>Tema, colores, tipografía, íconos y accesibilidad</h4></div>
        <div className="owner-config-list">
          ...
          <article><div><b>Accesibilidad</b><small>Control global como criterio de calidad, no como añadido.</small></div><State ok={profile.visualDesign.accessibilityControl} /></article>
        </div>
      </section>
    </div>
    ...
    <div className="owner-platform-columns">
      <section className="owner-platform-card">
        ...
        <div className="owner-config-list">
          <article><div><b>Cache seguro versionado</b><small>Browser 5 min, edge 10 min y stale-while-revalidate 24 h; secretos, auth, IA y endpoints propietarios nunca se cachean.</small></div><State ok={profile.cache.implemented} readyLabel="Activo" /></article>
      ...
      <section className="owner-platform-card">
        <div><span className="eyebrow">RELEASE Y COSTOS</span><h4>GitHub primero, deploy sólo cuando corresponda</h4></div>
        <div className="owner-config-list">
          <article><div><b>Fuente de verdad</b><small>Todo cambio debe quedar versionado en GitHub antes del despliegue.</small></div><State ok={profile.releasePolicy.pushAllChangesBeforeDeploy} /></article>
          <article><div><b>Mejoras agrupadas</b><small>Se validan en lote para evitar despliegues innecesarios.</small></div><State ok={profile.releasePolicy.batchValidatedImprovements} /></article>
          <article><div><b>Build y auditoría</b><small>Ambos deben estar en verde antes de considerar una versión lista.</small></div><State ok={profile.releasePolicy.requireGreenAudit && profile.releasePolicy.requireGreenBuild} /></article>
          <article><div><b>Producción automática</b><small>Desactivada; se habilita sólo mediante acción explícita de release.</small></div><State ok={!profile.releasePolicy.productionAutoDeploy} readyLabel="Bloqueada correctamente" /></article>
        </div>
      </section>
    </div>
