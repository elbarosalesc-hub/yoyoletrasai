...
const metrics = [
  {value: '28', detail: 'de 32 activos', label: 'Estudiantes', note: '', tone: 'violet', icon: Users},
  {value: '15', detail: 'actividades realizadas', label: 'Este mes', note: '', tone: 'mint', icon: Target},
  {value: '2h 35m', detail: 'tiempo de aprendizaje', label: 'Esta semana', note: '', tone: 'blue', icon: Clock3},
  {value: '12', detail: 'logros obtenidos', label: '¡Sigue así!', note: '', tone: 'yellow', icon: Medal}
]
  ...
  return (
    <article className={`metric-card tone-${metric.tone}`}>
      <div className="metric-top">
        <span className="metric-icon"><Icon size={28} aria-hidden="true" /></span>
        <div><strong>{metric.value}</strong><p>{metric.detail}</p></div>
      </div>
    ...
    <main className="app-frame">
      <aside className="sidebar" aria-label="Navegación principal">
        ...
        <nav className="side-nav">
          {navigation.map(({label, icon: Icon, active}) => (
            <a className={active ? 'nav-link active' : 'nav-link'} href="#" key={label} aria-current={active ? 'page' : undefined}>
              <Icon size={20} aria-hidden="true" /><span>{label}</span>
            </a>
          ))}
        </nav>
        <button className="profile-card" type="button">
          <span className="profile-avatar">Y</span><span><strong>YoYo Profe</strong><small>Docente</small></span><ChevronRight size={18} />
        </button>
      </aside>
      ...
      <section className="dashboard-shell">
        <header className="mobile-header">
          ...
          <div className="brand compact"><span className="brand-mark">Y</span><div><strong>YOYOLETRASAI</strong><small>Panel docente</small></div></div>
          <button className="notification-button" aria-label="3 notificaciones"><Bell size={21}/><span>3</span></button>
        </header>

        <header className="dashboard-header">
          <div><h1>¡Bienvenida, YoYo Profe! 👋</h1><p>Hoy es un gran día para crear experiencias de aprendizaje.</p></div>
          <div className="header-actions">
            <button className="notification-button" aria-label="3 notificaciones"><Bell size={21}/><span>3</span></button>
            <button className="primary-button"><span>Crear actividad</span><Plus size={22}/></button>
          </div>
        </header>
        ...
        <section className="featured-game">
          ...
          <div className="featured-copy">
            <span className="featured-badge">JUEGO INMERSIVO DESTACADO</span>
            ...
            <div className="chips"><span>Lenguaje</span><span>3.º básico</span><span>Comprensión</span><span>⭐ Niveles: 5</span></div>
            <div className="featured-actions"><button className="play-button"><Play size={20} fill="currentColor"/>Iniciar juego</button><button className="preview-button"><Eye size={20}/>Vista previa</button></div>
          </div>
        ...
        <section className="secondary-grid">
          <article className="panel-card upcoming-panel">
            <div className="section-heading"><h2>Próximas actividades</h2><a href="#">Ver todas</a></div>
            <div className="activity-list">
              <article className="activity-card"><span className="activity-art story">📚</span><div><strong>Comprensión lectora:<br/>Cuentos</strong><small>3.º básico · Lenguaje</small><span><CalendarDays size={14}/>20 de mayo</span></div></article>
              <article className="activity-card"><span className="activity-art puzzle">🧩</span><div><strong>Secuencia de eventos</strong><small>3.º básico · Lectura</small><span><CalendarDays size={14}/>21 de mayo</span></div></article>
            </div>
          </article>

          <article className="panel-card quick-panel">
            <div className="section-heading"><h2>Acciones rápidas</h2></div>
            <div className="quick-grid">{quickActions.map(({label, icon: Icon, tone}) => <button className="quick-action" key={label}><span className={`quick-icon tone-${tone}`}><Icon/></span><span>{label}</span></button>)}</div>
          </article>
        </section>
      </section>

      <nav className="mobile-bottom-nav" aria-label="Navegación móvil">
        {navigation.slice(0,5).map(({label, icon: Icon, active}) => <a href="#" key={label} className={active ? 'active' : ''}><Icon/><span>{label}</span></a>)}
      </nav>
    </main>
  )
}
