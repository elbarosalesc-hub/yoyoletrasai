...
async function loadInstitutionalContext(supabase:any, organizationId:string, courseId:string, studentId:string, objectiveId:string) {
  ...
  for (const row of evidence) {
    const key = String(row.achievement_level) as keyof typeof counts
    if (key in counts) counts[key] += 1
  }

  const parts = [
    `CONTEXTO INSTITUCIONAL VERIFICADO: Curso ${course.name}, nivel ${course.level}, año ${course.academic_year}.`,
    `Matrícula activa registrada: ${studentIds.size} estudiante(s).`,
    `Evidencias recientes del curso: ${evidence.length}. Distribución observada: logrado ${counts.achieved}, en desarrollo ${counts.developing}, inicial ${counts.initial}, no observado ${counts.not_observed}.`,
  ]

  if (selectedObjective) {
    parts.push(`OA/HABILIDAD SELECCIONADA: ${selectedObjective.code} · ${selectedObjective.title}. ${selectedObjective.description || ''}`)
  } else if (objectives.length) {
    parts.push(`OA/HABILIDADES disponibles en el curso: ${objectives.slice(0,8).map((item:any)=>`${item.code} ${item.title}`).join(' | ')}`)
  }
  ...
  if (studentId && studentIds.has(studentId)) {
    ...
    if (support) {
      if (support.strengths) parts.push(`Fortalezas: ${clean(support.strengths,700)}`)
      if (support.barriers) parts.push(`Barreras observadas: ${clean(support.barriers,700)}`)
      if (support.interests) parts.push(`Intereses útiles para contextualizar: ${clean(support.interests,500)}`)
      if (support.access_accommodations) parts.push(`Apoyos de acceso: ${clean(support.access_accommodations,700)}`)
      if (support.objective_accommodations) parts.push(`Ajustes de objetivo registrados: ${clean(support.objective_accommodations,700)}`)
      if (support.assistive_technology) parts.push(`Tecnología de apoyo: ${clean(support.assistive_technology,500)}`)
      if (support.evidence_notes) parts.push(`Notas pedagógicas de evidencia: ${clean(support.evidence_notes,700)}`)
    }
    if (recent.length) {
      parts.push(`Evidencias individuales recientes: ${recent.map((item:any)=>`${item.observed_at}: ${clean(item.description,350)} [logro ${item.achievement_level}; autonomía ${item.autonomy_level || 'sin registro'}; apoyo ${clean(item.support_used,180)||'sin registro'}]`).join(' | ')}`)
    }
  }
...
export async function POST(request:Request){
  ...
  const studentId=safeId(body.studentId)
  const objectiveId=safeId(body.objectiveId)
  ...
  try{
    ...
    return NextResponse.json({result,model,fallback:false,contextUsed:Boolean(institutionalContext)})
  }catch(error){
    const result=fallback(mode,level,subject,prompt,support)
