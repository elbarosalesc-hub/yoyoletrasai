...
export default function ResetPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccess(true)
      setFeedback('Tu contraseña fue actualizada correctamente.')
    } catch {
      setFeedback('No fue posible actualizar la contraseña. Solicita un nuevo enlace de recuperación.')
    } finally {
      setPending(false)
    }
  }
  ...
  return (
    <main className="reset-shell">
      <section className="reset-card">
        <div className="reset-brand">YO</div>
        <span>SEGURIDAD DE CUENTA</span>
        <h1>Crear nueva contraseña</h1>
        <p>Define una contraseña segura para volver a ingresar a YOYOLETRASAI.</p>

        {feedback && <div className={`feedback ${success ? 'success' : ''}`} role="status">{feedback}</div>}
