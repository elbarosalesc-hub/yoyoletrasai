    ...
    try {
      ...
      if (error) {
        ...
        if (code === 'same_password' || message.includes('different from the old password')) {
          setSuccess(true)
          setFeedback('Esta ya es tu contraseña actual. Tu acceso está listo y puedes entrar al panel de propietaria.')
          return
        }
        if (message.includes('session') || message.includes('jwt') || message.includes('token')) {
          setFeedback('La sesión de activación venció. Solicita un nuevo enlace desde “Crear mi contraseña”.')
          return
        }
        throw error
      }
