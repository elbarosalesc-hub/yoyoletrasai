...
type HealthPayload = {
  ...
  checkedAt?: string
  services?: { application?: 'ok'; databaseGateway?: DatabaseGatewayStatus }
}
...
export function StatusMonitor() {
  ...
  const checkHealth = useCallback(async () => {
    ...
    try {
      ...
      setPayload(result)
      if (result.status === 'ok') setMessage('Los servicios esenciales responden correctamente.')
      else if (result.status === 'misconfigured') setMessage('La aplicación responde, pero faltan variables de configuración esenciales.')
      else setMessage('La plataforma responde con capacidad limitada. Revisa la conectividad de datos.')
    } catch {
      setPayload(null)
  ...
  return (
    <section className={styles.monitor} aria-live="polite">
      <div className={`${styles.summary} ${overallOk ? styles.summaryOk : checkFailed ? styles.summaryError : styles.summaryPending}`}>
        <span className={styles.summaryIcon}>{overallOk ? <CheckCircle2 size={28} /> : <TriangleAlert size={28} />}</span>
        <div>
      ...
      <div className={styles.grid}>
        ...
        <StatusCard icon={<Database size={22} />} title="Supabase" value={databaseLabel(databaseValue, loading)} ok={databaseOk} />
      </div>
...
function StatusCard({ icon, title, value, ok }: { icon: ReactNode; title: string; value: string; ok: boolean }) {
  return <article className={styles.card}><span className={styles.cardIcon}>{icon}</span><div><small>{title}</small><strong>{value}</strong></div><em className={ok ? styles.good : styles.review}>{ok ? 'Operativo' : 'Revisar'}</em></article>
}
