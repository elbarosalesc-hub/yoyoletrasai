...
export default function Cursos(){
 ...
 const filtered=courses.filter(course=>course.name.toLowerCase().includes(search.toLowerCase()))
 const createCourse=()=>{const name=newName.trim();if(!name)return;const next:Course={id:Date.now(),name,students:Math.max(1,Number(newStudents)||1),progress:0,oa:0,groups:[{name:'Avance autónomo',detail:'Profundización y producción',students:0},{name:'Avance con apoyo',detail:'Modelamiento y pistas',students:0},{name:'Apoyo intensivo',detail:'Mediación y apoyos visuales',students:0}]};setCourses(value=>[...value,next]);setSelectedId(next.id);setNewName('');setNewStudents('30');setShowCreate(false)}
 const adjustGroup=(index:number,delta:number)=>setCourses(list=>list.map(course=>course.id!==selectedId?course:{...course,groups:course.groups.map((group,groupIndex)=>groupIndex===index?{...group,students:Math.max(0,group.students+delta)}:group)}))

 return <ModuleShell active="Cursos y grupos">
  <section className="module-page-v2 legacy-module-v2">
   <div className="page-head"><div><span className="module-eyebrow">Organización del aula</span><h1>Cursos y grupos</h1><p>Organiza estudiantes, apoyos, objetivos y grupos flexibles desde un solo lugar.</p></div><div className="course-toolbar"><div className="search premium-search" style={{display:'flex'}}><Search size={17}/><input aria-label="Buscar curso" placeholder="Buscar curso..." value={search} onChange={event=>setSearch(event.target.value)}/></div><button className="btn btn-primary" onClick={()=>setShowCreate(value=>!value)}><Plus size={17}/> Crear curso</button></div></div>
   {showCreate&&<section className="panel course-create-panel"><div className="course-create-grid"><label>Nombre del curso<input value={newName} onChange={event=>setNewName(event.target.value)} placeholder="Ej.: 4.º Básico B"/></label><label>Estudiantes<input type="number" min="1" value={newStudents} onChange={event=>setNewStudents(event.target.value)}/></label><label>Año escolar<select><option>2026</option><option>2027</option></select></label><button className="btn btn-primary" onClick={createCourse}>Guardar curso</button></div></section>}
