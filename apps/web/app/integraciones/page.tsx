 ...
 return <AppShell active="Integraciones">
  ...
  <div className="integration-layout">
   ...
   <section className="integration-main premium-card"><div className="integration-title"><div className={`provider-logo large ${selected}`}>{selected==='canvas'?'C':'G'}</div><div><span>Integración institucional</span><h2>{provider.name}</h2><p>{provider.description}</p></div><span className="connection-state"><Unplug size={16}/>Sin conectar</span></div>
    ...
    <div className="permission-list"><h3>Permisos propuestos</h3>{selected==='canvas'?['Leer cursos y matrículas autorizadas','Crear o actualizar tareas seleccionadas','Leer entregas y resultados cuando la docente lo active']:['Leer cursos y listas autorizadas','Crear materiales y trabajos de clase','Leer entregas y calificaciones cuando la docente lo active'].map(x=>null)}{(selected==='canvas'?['Leer cursos y matrículas autorizadas','Crear o actualizar tareas seleccionadas','Leer entregas y resultados cuando la docente lo active']:['Leer cursos y listas autorizadas','Crear materiales y trabajos de clase','Leer entregas y calificaciones cuando la docente lo active']).map(x=><div key={x}><CheckCircle2 size={17}/><span>{x}</span></div>)}</div>
