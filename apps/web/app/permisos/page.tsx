...
const permissionLabels=[
 ['view_dashboard','Ver panel principal'],['create_resources','Crear recursos'],['manage_students','Gestionar estudiantes'],['view_reports','Ver informes'],['manage_users','Gestionar usuarios'],['play_games','Acceder a juegos 3D'],['view_family_portal','Ver portal familiar'],['use_virtual_teacher','Usar profesor virtual']
] as const
...
export default function PermissionsPage(){
 const[enabled,setEnabled]=useState<Record<string,boolean>>(()=>Object.fromEntries(roles.flatMap(role=>permissionLabels.map(([key])=>[`${role}:${key}`,role==='admin'||role==='teacher'||(role==='student'&&['view_dashboard','play_games','use_virtual_teacher'].includes(key))||(role==='guardian'&&key==='view_family_portal')])) )
