  ...
  return <ModuleShell active="Inicio">
    <div className="dashboard-content canonical-dashboard">
      ...
      <section className="lower-grid">
        ...
        <article className="panel-card quick-panel"><div className="section-heading"><div><span className="section-kicker">HERRAMIENTAS</span><h2>Acciones rápidas</h2></div></div><div className="quick-grid">{quickActions.map(({label,description,icon:Icon,tone,href})=><a className="quick-action" href={href} key={label}><span className={`quick-icon tone-${tone}`}><Icon/></span><span><strong>{label}</strong><small>{description}</small></span><ChevronRight size={17}/></a>)}</div></article>
      </section>
    </div>
  </ModuleShell>
}
