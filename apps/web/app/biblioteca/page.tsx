...
type MissionSummary={id:string;source_href:string|null;status:string}
...
export default function Biblioteca(){
 ...
 const[serverMissions,setServerMissions]=useState<MissionSummary[]>([])
 ...
 const handleSaved=(assignment:ResourceAssignment)=>{
  setAssignments(current=>[assignment,...current])
  setServerMissions(current=>[{id:assignment.id,source_href:`/biblioteca/${assignment.activitySlug}`,status:'assigned'},...current])
 }

 return <AppShell active="Biblioteca Premium">
  <section className="premium-hero library-hero"><span className="eyebrow">Biblioteca inteligente</span><h1>Recursos listos para enseñar, adaptar y asignar</h1><p>Explora actividades con objetivos, secuencia, apoyos y criterios de logro. Cada asignación crea una Misión YOYO con seguimiento institucional real.</p><div className="library-stats"><span><strong>{resourceCatalog.length}</strong> recursos</span><span><strong>{favorites.length}</strong> favoritos</span><span><strong>{assignedSlugs.size}</strong> recursos en misiones</span></div></section>
  ...
  {filtered.length===0?<section className="library-empty premium-card"><h2>No encontramos recursos</h2><p>Prueba quitando uno de los filtros o busca por otra habilidad.</p><button className="btn btn-primary" onClick={clearFilters}>Ver todos los recursos</button></section>:<div className={`premium-library-grid library-${view}`}>{filtered.map((resource,index)=>{
   ...
   const premiumPilot=resource.slug==='trazos-animales'
   return <article className={`premium-resource-card tone-${index%6}`} key={resource.slug}>
    <div className="premium-card-art">{premiumPilot&&<span className="premium-label">Premium contextual</span>}<span>{art[resource.illustration]||'✨'}</span><em>{resource.format}</em><button className={`resource-favorite ${favorite?'active':''}`} onClick={()=>toggleFavorite(resource.slug)} aria-label={favorite?'Quitar de favoritos':'Guardar en favoritos'}><Bookmark/></button></div>
    <div className="premium-card-body"><div className="premium-card-meta"><span>{resource.subject}</span><b>{resource.oa}</b></div><h3>{resource.title}</h3><p>{resource.summary}</p>{premiumPilot&&<div className="premium-card-tags"><span>Currículum chileno</span><span>DUA</span><span>3 niveles</span><span>8 láminas</span><span>Bajo costo</span></div>}<div className="premium-card-info"><span>{resource.level}</span><span>{resource.duration}</span></div>{assigned&&<div className="resource-assigned"><CheckCircle2 size={15}/>Asignado como Misión YOYO</div>}<div className="resource-actions"><button className="btn btn-soft" onClick={()=>setPreview(resource)}><Eye size={16}/>Vista rápida</button><button className="btn btn-soft" onClick={()=>setAssignmentTarget(resource)}><Users size={16}/>{assigned?'Asignar de nuevo':'Asignar'}</button><Link className="btn btn-primary" href={`/biblioteca/${resource.slug}`}>Abrir</Link><Link className="btn btn-soft" href={`/crear?adaptar=${resource.slug}`}>Adaptar</Link></div></div>
   </article>
  })}</div>}

  {preview?<div className="resource-preview-backdrop" role="presentation" onMouseDown={()=>setPreview(null)}><aside className="resource-preview-drawer" role="dialog" aria-modal="true" aria-label={`Vista previa de ${preview.title}`} onMouseDown={event=>event.stopPropagation()}><button className="resource-preview-close" onClick={()=>setPreview(null)} aria-label="Cerrar vista previa"><X/></button><div className="resource-preview-hero"><span>{art[preview.illustration]||'✨'}</span><em>{preview.format}</em></div><div className="resource-preview-content"><span className="eyebrow">{preview.subject} · {preview.level}</span><h2>{preview.title}</h2><p>{preview.summary}</p>{preview.slug==='trazos-animales'&&<div className="premium-card-tags"><span>Contexto cultural chileno</span><span>Bienestar emocional</span><span>Materiales accesibles</span></div>}<div className="resource-preview-facts"><div><b>{preview.oa}</b><small>Objetivo</small></div><div><b>{preview.duration}</b><small>Duración</small></div><div><b>{preview.format}</b><small>Formato</small></div></div><section><h3>Uso pedagógico sugerido</h3><p>{preview.slug==='trazos-animales'?'Activa el cuerpo, modela un solo recorrido y permite elegir herramienta, nivel de apoyo y forma de participación. Cierra reconociendo la estrategia que ayudó al estudiante.':'Presenta el propósito, modela el primer paso y permite respuesta oral, escrita o manipulativa según las necesidades del grupo.'}</p></section><div className="resource-preview-actions"><Link className="btn btn-primary" href={`/biblioteca/${preview.slug}`}>Abrir recurso completo</Link><button className="btn btn-soft" onClick={()=>{setAssignmentTarget(preview);setPreview(null)}}><Users/>Asignar como misión</button><Link className="btn btn-soft" href={`/crear?adaptar=${preview.slug}`}>Crear adaptación</Link></div></div></aside></div>:null}

  <ResourceAssignmentModal activity={assignmentTarget} open={Boolean(assignmentTarget)} onClose={()=>setAssignmentTarget(null)} onSaved={handleSaved}/>
 </AppShell>
}
