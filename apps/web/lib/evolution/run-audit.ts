...
export async function runEvolutionAudit(db: EvolutionDb, organizationId: string, triggeredBy: EvolutionTrigger) {
  ...
  try {
    ...
    const [resources, generations, evals, benchmarks, findings] = await Promise.all([
      db.from('premium_resources').select('id,status,quality_score,accessibility_score').eq('organization_id', organizationId),
    ...
    const accessibilityValues = published.map(row => Number(row.accessibility_score)).filter(Number.isFinite)
    ...
    const metrics = {
      ...
      resources: { total: resourceRows.length, published: published.length, averageQuality: average(qualityValues), averageAccessibility: average(accessibilityValues) },
    ...
    if (gameCoverage < 70) propose('games','Aumentar variedad real de juegos',`Sólo ${playableGames} de ${gameExperiences.length} experiencias están jugables.`,'Construir mecánicas realmente distintas: exploración, clasificación, precisión, laboratorio, escape room y desafío colaborativo; incluir niveles, feedback, audio opcional y accesibilidad.',90,95,80,35)
    ...
    if (published.length < 20) propose('resources','Ampliar biblioteca premium validada',`Hay ${published.length} recursos publicados.`,'Generar candidatos en borrador según brechas curriculares y aprobar sólo los que superen calidad, accesibilidad y originalidad.',86,90,65,25)
