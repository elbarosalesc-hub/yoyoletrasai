...
import {BookOpen,Home,Sparkles,Users,Gamepad2,Accessibility,BarChart3,Settings,Search,PenTool,Images,ShieldCheck,ClipboardList,FlaskConical,FileText,UsersRound,Menu,Wrench,Bell,Mail,Bot,Cloud} from 'lucide-react'
...
export function AppShell({children,active}:{children:React.ReactNode;active:string}){
 return <div className="app-shell premium-shell">
  <aside className="sidebar premium-sidebar">
   ...
   <nav className="menu premium-menu" aria-label="Navegación principal">{items.map(([label,href,Icon])=><Link key={label} href={href} className={label===active?'active':''}><Icon size={17}/><span>{label}</span>{label==='Juegos inmersivos'&&<em>3D</em>}</Link>)}</nav>
   <div className="sidebar-foot premium-plan"><strong>👑 Plan Premium</strong><span>Herramientas pedagógicas integradas</span><Link href="/qa">Estado del sistema</Link></div>
  </aside>
  <main className="app-main premium-main" id="contenido-principal">
   <div className="app-top premium-topbar">
    ...
    <div className="search premium-search" role="search"><Search size={18}/><span>Buscar actividades, temas, OA, habilidades...</span></div>
