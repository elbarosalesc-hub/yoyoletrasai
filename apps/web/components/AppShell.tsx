...
import {BookOpen,Home,Sparkles,Users,Gamepad2,Accessibility,BarChart3,Settings,Search,PenTool,Images,ShieldCheck,ClipboardList,FlaskConical,FileText,UsersRound,Menu,Wrench,Bell,Mail,Bot} from 'lucide-react'
...
const mobile=[items[0],items[1],items[2],items[3],items[5],items[6],items[16]] as const
...
export function AppShell({children,active}:{children:React.ReactNode;active:string}){
 return <div className="app-shell premium-shell">
  ...
  <nav className="mobile-nav premium-mobile-nav">{mobile.map(([label,href,Icon])=><Link key={label} href={href} className={label===active?'active':''}><Icon size={20}/><span>{label==='Crear con IA'?'Crear':label==='Juegos inmersivos'?'Juegos':label==='Caligrafía y trazos'?'Trazos':label==='Profesor Virtual'?'YOYO':label}</span></Link>)}</nav>
 </div>
}