...
export default function ThreeFair({reducedMotion=false,highContrast=false,onReady}:Props){
 ...
 useEffect(()=>{
  ...
  const addStall=(x:number,z:number,color:number,awning:number,index:number)=>{const g=new THREE.Group();const base=new THREE.Mesh(new THREE.BoxGeometry(3.5,1.35,2.35),new THREE.MeshStandardMaterial({color,roughness:.72}));base.position.y=.68;base.castShadow=true;g.add(base);const roof=new THREE.Mesh(new THREE.BoxGeometry(3.95,.18,2.8),new THREE.MeshStandardMaterial({color:awning,roughness:.62}));roof.position.y=2.18;roof.castShadow=true;g.add(roof);for(const sx of[-1.48,1.48]){const post=new THREE.Mesh(new THREE.CylinderGeometry(.07,.07,1.55,10),new THREE.MeshStandardMaterial({color:0x76513a}));post.position.set(sx,1.47,.92);g.add(post)}for(let i=0;i<7;i++){const item=new THREE.Mesh(new THREE.SphereGeometry(.18+(i%3)*.035,16,16),new THREE.MeshStandardMaterial({color:[0xffd45a,0x7ecf62,0xff8e65,0x7fb7ff][(i+index)%4],roughness:.68}));item.position.set(-1.2+i*.4,1.52,.38+(i%2)*.1);item.castShadow=true;g.add(item)}const sign=new THREE.Mesh(new THREE.BoxGeometry(1.4,.5,.08),new THREE.MeshStandardMaterial({color:0xffffff,emissive:highContrast?0x222222:0x000000,emissiveIntensity:.08}));sign.position.set(0,2.65,.05);g.add(sign);g.position.set(x,0,z);scene.add(g);stalls.push(g)}
  addStall(-5,-4,stallColors[0],0xffefe9,0);addStall(5,-4,stallColors[1],0xe8fff9,1);addStall(-5,-10,stallColors[2],0xfff7dc,2);addStall(5,-10,stallColors[3],0xf1edff,3)
 ...
 return <div ref={mountRef} className="webgl-forest" aria-label="Feria escolar tridimensional animada con puestos, productos, personajes, globos y caja interactiva"/>
}
