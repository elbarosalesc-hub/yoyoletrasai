...
type Interactive={id:string;mesh:THREE.Object3D;highlight:THREE.Object3D}

export default function ThreeForest({reducedMotion=false,highContrast=false,active=[],onReady,onSelect}:Props){
 ...
 useEffect(()=>{
  ...
  try{
   ...
   setFallback(true);onReady?.(false);return
  }
  ...
  scene.background=new THREE.Color(highContrast?0x071923:0x8fd6ee)
  scene.fog=new THREE.FogExp2(highContrast?0x071923:0xb9dfd5,.035)
  ...
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(38,38,32,32),new THREE.MeshStandardMaterial({color:highContrast?0x13492c:0x477f45,roughness:.95}));ground.rotation.x=-Math.PI/2;ground.receiveShadow=true;scene.add(ground)
  ...
  const streamMat=new THREE.MeshStandardMaterial({color:highContrast?0x4ce3ff:0x59b9d3,roughness:.18,metalness:.08,transparent:true,opacity:.86})
  const stream=new THREE.Mesh(new THREE.PlaneGeometry(3.1,26),streamMat);stream.rotation.x=-Math.PI/2;stream.rotation.z=.18;stream.position.set(-7,.04,-6);scene.add(stream)
  ...
  const makeHalo=(x:number,y:number,z:number,r=.62)=>{const mat=new THREE.MeshBasicMaterial({color:highContrast?0xffffff:0xffdc78,transparent:true,opacity:.18,side:THREE.DoubleSide,depthWrite:false});const halo=new THREE.Mesh(new THREE.RingGeometry(r,r+.12,32),mat);halo.position.set(x,y,z);halo.rotation.x=-Math.PI/2;scene.add(halo);return halo}

  const cabin=new THREE.Group();const wallMat=new THREE.MeshStandardMaterial({color:highContrast?0xd67c45:0xb9683e,roughness:.88,emissive:active.includes('cabana')?0x3a1609:0x000000,emissiveIntensity:active.includes('cabana')?.9:0});const body=new THREE.Mesh(new THREE.BoxGeometry(3.8,2.6,3.2),wallMat);body.position.y=1.3;body.castShadow=true;cabin.add(body);const roof=new THREE.Mesh(new THREE.ConeGeometry(3.2,1.75,4),new THREE.MeshStandardMaterial({color:0x6c4132,roughness:.9}));roof.rotation.y=Math.PI/4;roof.position.y=3.3;roof.castShadow=true;cabin.add(roof);const door=new THREE.Mesh(new THREE.BoxGeometry(.82,1.55,.12),new THREE.MeshStandardMaterial({color:0x4e342d}));door.position.set(0,.78,1.66);cabin.add(door);[-1.05,1.05].forEach(x=>{const windowMesh=new THREE.Mesh(new THREE.BoxGeometry(.62,.62,.1),new THREE.MeshStandardMaterial({color:highContrast?0xfff06a:0xffd27d,emissive:0x8a4f13,emissiveIntensity:.65}));windowMesh.position.set(x,1.58,1.67);cabin.add(windowMesh)});cabin.position.set(4.2,0,-8.2);cabin.userData.interactiveId='cabana';scene.add(cabin);interactive.push({id:'cabana',mesh:cabin,highlight:makeHalo(4.2,.08,-6.55,1.05)})
  ...
  const backpackMat=new THREE.MeshStandardMaterial({color:0xf08a55,roughness:.7,emissive:active.includes('mochila')?0x6b2300:0x000000,emissiveIntensity:active.includes('mochila')?1.15:0})
  const backpack=new THREE.Mesh(new THREE.BoxGeometry(.78,.92,.38),backpackMat);backpack.position.set(-1.45,.5,.6);backpack.rotation.y=.32;backpack.castShadow=true;backpack.userData.interactiveId='mochila';scene.add(backpack);interactive.push({id:'mochila',mesh:backpack,highlight:makeHalo(-1.45,.07,.6,.55)})

  const noteMat=new THREE.MeshStandardMaterial({color:0xfff6cf,roughness:1,emissive:active.includes('nota')?0x8b7000:0x000000,emissiveIntensity:active.includes('nota')?.8:0})
  const note=new THREE.Mesh(new THREE.BoxGeometry(.72,.05,.52),noteMat);note.position.set(.55,.08,-.55);note.rotation.y=-.35;note.userData.interactiveId='nota';scene.add(note);interactive.push({id:'nota',mesh:note,highlight:makeHalo(.55,.07,-.55,.5)})

  const bird=new THREE.Group();const birdBody=new THREE.Mesh(new THREE.SphereGeometry(.22,18,18),new THREE.MeshStandardMaterial({color:highContrast?0xffffff:0x315d83,emissive:active.includes('ave')?0x153f5c:0x000000,emissiveIntensity:active.includes('ave')?.85:0}));birdBody.scale.set(1.15,.75,.8);bird.add(birdBody);const wing=new THREE.Mesh(new THREE.ConeGeometry(.18,.42,3),new THREE.MeshStandardMaterial({color:0x1f4565}));wing.rotation.z=Math.PI/2;wing.position.set(.05,0,.15);bird.add(wing);bird.position.set(2.65,3.55,-4.2);bird.userData.interactiveId='ave';scene.add(bird);interactive.push({id:'ave',mesh:bird,highlight:makeHalo(2.65,3.25,-4.2,.42)})

  interactive.forEach(item=>{const mat=(item.highlight as THREE.Mesh).material as THREE.MeshBasicMaterial;mat.opacity=active.includes(item.id)?.5:.18})
  ...
  const pick=()=>{raycaster.setFromCamera(pointer,camera);const candidates=interactive.flatMap(item=>item.mesh.children.length?[item.mesh,...item.mesh.children]:[item.mesh]);const hits=raycaster.intersectObjects(candidates,true);if(!hits.length)return null;let node:THREE.Object3D|null=hits[0].object;while(node&&!node.userData.interactiveId)node=node.parent;return node?.userData.interactiveId as string|undefined}
  const onPointerMove=(event:PointerEvent)=>{setPointer(event);renderer.domElement.style.cursor=pick()?'pointer':'grab'}
  const onPointerDown=(event:PointerEvent)=>{setPointer(event);const id=pick();if(id){renderer.domElement.style.cursor='pointer';onSelect?.(id)}}
  ...
  const animate=()=>{const t=clock.getElapsedTime();if(!reducedMotion){guide.position.y=Math.sin(t*1.6)*.045;guide.rotation.y=Math.sin(t*.55)*.12;armR.rotation.z=.35+Math.sin(t*2.2)*.18;bird.position.y=3.55+Math.sin(t*2.1)*.08;bird.rotation.y=Math.sin(t*.8)*.28;wing.rotation.y=Math.sin(t*8)*.55;trees.forEach((g,i)=>g.rotation.z=Math.sin(t*.38+i*.73)*.009);fireflies.forEach(f=>{f.mesh.position.y=f.baseY+Math.sin(t*f.speed+f.phase)*f.radius;f.mesh.position.x+=Math.cos(t*f.speed*.7+f.phase)*.0015});butterflies.forEach((f,i)=>{f.mesh.position.y=f.baseY+Math.sin(t*f.speed*2+f.phase)*.28;f.mesh.position.x+=Math.cos(t*f.speed+f.phase)*.006;f.mesh.rotation.y=Math.sin(t*7+i)*.9});interactive.forEach((item,i)=>{const halo=item.highlight as THREE.Mesh;halo.scale.setScalar(1+Math.sin(t*2+i)*.08)});cloudGroup.position.x=((t*.18)%6)-3;streamMat.opacity=.8+Math.sin(t*1.8)*.05;camera.position.x+=(pointerX*.24-camera.position.x)*.018;camera.position.y+=(5.2-pointerY*.1-camera.position.y)*.018;camera.lookAt(0,1.5,-4)}renderer.render(scene,camera);raf=requestAnimationFrame(animate)}
 ...
 },[reducedMotion,highContrast,active,onReady,onSelect])
 ...
 return <div ref={mountRef} className="webgl-forest" aria-label="Escena tridimensional animada e interactiva del bosque con personaje, cabaña, mochila, nota, ave, agua y ambientación"/>
}
