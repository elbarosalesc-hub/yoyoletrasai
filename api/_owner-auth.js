...
export function createAdminClient() {
  ...
  if (!config.url || !config.secretKey) {
    throw new Error('La administración segura de Supabase no está configurada.');
  }
...
export async function authorizeOwnerRequest(req) {
  ...
  if (!token || !config.url || !config.publishableKey) {
    return { ok: false, status: 401, error: 'Inicia sesión como propietaria para continuar.' };
  }
  ...
  if (userError || !userData.user || !isOwnerAccountEmail(userData.user.email)) {
    return { ok: false, status: 403, error: 'Esta operación está reservada a la propietaria verificada.' };
  }
  ...
  if (membershipError || !membership) {
    return { ok: false, status: 403, error: 'El rol de superadministradora no está activo.' };
  }
