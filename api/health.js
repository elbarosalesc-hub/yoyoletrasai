...
export default function handler(_request, response) {
  const authentication = process.env.AI_GATEWAY_API_KEY ? 'gateway_key' : (process.env.YOYO_CREDENTIAL_ENCRYPTION_KEY || process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY) ? 'owner_managed_or_pending' : process.env.VERCEL_OIDC_TOKEN ? 'vercel_oidc' : 'not_configured';
  response.status(200).json({
    status: 'ok', service: 'YoYoLetrasAI', version: '3.6.0', environment: 'production',
    aiCredential: { id: YOYO_AI_PUBLIC_CREDENTIAL.credentialId, issuer: YOYO_AI_PUBLIC_CREDENTIAL.issuer, authentication },
    checks: {
      application: 'operational', resourcesApi: 'operational',
